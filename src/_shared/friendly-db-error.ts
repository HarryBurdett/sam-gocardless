...
const PATTERNS: Array<{ test: RegExp; msg: string }> = [
  {
    ...
    msg:
      'Opera database is currently unavailable — it may be locked by a backup or another process. Please try again in a few minutes.',
  },
  {
    test: /18456|login failed/i,
    msg:
      'Cannot connect to Opera — database login failed. Please check the connection settings.',
  },
  {
    test: /(timeout).*(connection|login)|(connection|login).*timeout/i,
    msg:
      'Connection to the Opera database timed out. Please try again shortly.',
  },
  {
    test: /(network|unreachable|tcp provider|server is not found)/i,
    msg:
      'Cannot reach the Opera database server. Please check the network connection.',
  },
  {
    test: /(deadlock|1205)/i,
    msg:
      'The operation was temporarily blocked by another user. Please try again.',
  },
  {
    test: /(lock request time out|lock timeout)/i,
    msg:
      'Opera is busy — another user or process is updating the same data. Please wait and try again.',
  },
  {
...
export function friendlyDbError(raw: unknown): string {
  if (raw === null || raw === undefined) return 'An unexpected error occurred.';
  const msg = (raw instanceof Error ? raw.message : String(raw)).trim();
  if (!msg) return 'An unexpected error occurred.';

  for (const p of PATTERNS) {
    if (p.test.test(msg)) return p.msg;
  }
  ...
  if (/database connection failed|query execution failed/i.test(msg) && msg.includes(': ')) {
    ...
    if (inner && inner !== msg) {
      ...
      if (innerFriendly !== inner) return innerFriendly;
    }
  }
