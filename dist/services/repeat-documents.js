...
export async function getRepeatDocuments(operaDb, appDb, opts = {}) {
    ...
    try {
        ...
        const subRows = (await appDb('gocardless_subscriptions')
            .select('subscription_id', 'opera_account', 'amount_pence', 'source_doc', 'status', 'name', 'interval_unit', 'interval_count'));
        ...
        const docLinks = (await appDb('gocardless_subscription_documents').select('subscription_id', 'source_doc'));
        const subBySourceDoc = new Map();
        ...
        for (const link of docLinks ?? []) {
            ...
            if (sub && trim(sub.status) !== 'cancelled') {
                ...
                if (!subBySourceDoc.has(doc))
                    subBySourceDoc.set(doc, sub);
            }
        }
        // 4. Read Opera ihead joined to itran totals (in pence).
        const heads = (await operaDb('ihead')
            .leftJoin(operaDb('itran')
            .select('it_doc')
            .sum({ line_nett: 'it_exvat' })
            .sum({ line_vat: 'it_vatval' })
            .groupBy('it_doc')
            .as('lines'), 'lines.it_doc', 'ih_doc')
            .where({ ih_docstat: 'U' })
            .andWhere((qb) => {
            qb.whereNull('ih_econtr').orWhere('ih_econtr', '>=', operaDb.raw('GETDATE()'));
        })
            .orderBy([
            { column: 'ih_account', order: 'asc' },
            { column: 'ih_doc', order: 'asc' },
        ])
            .select('ih_doc', 'ih_account', 'ih_name', 'ih_ignore', 'ih_dcontr', 'ih_scontr', 'ih_econtr', 'ih_job', 'ih_analsys', 'ih_custref', 'ih_narr1', operaDb.raw('COALESCE(lines.line_nett, 0) AS line_nett'), operaDb.raw('COALESCE(lines.line_vat, 0) AS line_vat')));
        const documents = [];
        for (const row of heads ?? []) {
            ...
            if (existingSub) {
                ...
                if (subAmount !== amountPence) {
                    details.push(`Amount: subscription ${formatPounds(subAmount)} vs document ${formatPounds(amountPence)}`);
                }
                if (subUnit !== freq.unit || subCount !== freq.count) {
                    ...
                    const subFrequencyLabel = subUnit === 'monthly' && subCount === 3
                        ? 'Quarterly'
                    ...
                    details.push(`Frequency: subscription ${subFrequencyLabel} vs document ${freq.label}`);
                }
                if (details.length > 0) {
                    mismatch = {
                        details,
            ...
            documents.push({
                ...
                matching_subscription: matchingSub
                    ? {
                        ...
                        amount_formatted: formatPounds(Math.round(Number(matchingSub.amount_pence ?? 0))),
...
//# sourceMappingURL=repeat-documents.js.map