...
export async function fetchGocardlessApiPayouts(appDb, operaDb, client, environment, input) {
    ...
    const orphanPayoutIds = new Set();
    const orphanReferences = new Set();
    for (const o of orphanResult.orphans ?? []) {
        if (o.payout_id)
            orphanPayoutIds.add(o.payout_id);
        if (o.bank_reference)
            orphanReferences.add(o.bank_reference);
    }
    // === EARLY FILTERING ===
    ...
    for (const payout of rawPayouts) {
        ...
        try {
            if (!orphanPayoutIds.has(payout.id) &&
    ...
    const orphanCheck = orphanResult.success && (orphanResult.orphans?.length ?? 0) > 0
        ? {
            detected: true,
