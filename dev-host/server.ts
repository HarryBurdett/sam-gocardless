/**
 * Local dev host that loads the GoCardless SAM plugin standalone.
 *
 ...
 * or live GoCardless API. `db.getCompanyDb()` returns null, so any
 * route that touches Opera will surface a 500/null error.
 */
...
async function main() {
  ...
  app.use('/api/apps/gocardless', (req, _res, next) => {
    req.user = {
      userId: 'dev-user',
  ...
  app.listen(port, () => {
    ...
    console.log(`[dev-host] plugin API:  /api/apps/gocardless/*`);
    console.log(`[dev-host] frontend:    /api/apps/gocardless/static/index.js`);
  });
}
