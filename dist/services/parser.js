...
export function detectTransactionCurrency(content) {
    ...
    for (const rawLine of content.split('\n')) {
        ...
        if (summaryKeywords.some((kw) => lower.includes(kw)))
            continue;
        if (lower.includes('customer') && lower.includes('description'))
            continue;
        const m = /[\d,]+\.?\d+\s*(GBP|EUR|USD|CAD|AUD)\s*$/i.exec(line);
        const code = m?.[1];
        if (code)
            paymentCurrencies.push(code.toUpperCase());
    }
...
export function detectPayoutCurrency(content) {
    ...
    if (netMatch?.[1])
        return netMatch[1].toUpperCase();
    return 'GBP';
}
export function detectCurrency(content) {
    const tx = detectTransactionCurrency(content);
    if (tx !== 'GBP')
        return tx;
    return detectPayoutCurrency(content);
}
export function extractInvoiceRefs(description) {
...
function inferYearForMonth(monthIdx) {
    const now = new Date();
    const currentYear = now.getFullYear();
    const currentMonth = now.getMonth();
    return monthIdx <= currentMonth + 1 ? currentYear : currentYear - 1;
}
export function parseGocardlessEmail(content) {
    const payments = [];
    let gross_amount = 0;
    ...
    for (let i = 0; i < cleanedLines.length; i++) {
        ...
        if (trimmedLower.includes('customer') &&
        ...
        if (trimmedLower === 'amount') {
            ...
            if (headerPartsSeen.has('customer') &&
