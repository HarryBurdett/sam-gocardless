...
function refSuffix(ref: string): string {
  if (!ref) return '';
  if (ref.includes('-')) return ref.split('-').pop() as string;
  return ref.slice(-8);
}
...
function formatDate(d: string | Date | null | undefined): string {
  ...
  if (y && m && dy) return `${dy}/${m}/${y}`;
  return s;
}

function bankFilterSqlParts(
  gcBankCode: string | null | undefined,
  destBankCode: string | null | undefined,
...
async function findOperaByReference(
  operaDb: Knex,
  refSuffixValue: string,
  bankFilter: { sql: string; params: string[] },
): Promise<OperaDupRow | null> {
  if (!refSuffixValue) return null;
  try {
    const rows = (await operaDb.raw(
      ...
      [`%${refSuffixValue}%`, ...bankFilter.params],
    )) as unknown as OperaDupRow[];
 ...
 */
async function findOperaByGrossDate(
  operaDb: Knex,
  grossPence: number,
  payoutDate: string,
  bankFilter: { sql: string; params: string[] },
): Promise<OperaDupRow | null> {
  if (!grossPence || !payoutDate) return null;
  try {
    const rows = (await operaDb.raw(
      `SELECT TOP 1 ae_value, at_pstdate AS at_date, ae_entref
        ...
        WHERE at_type IN (1, 4, 6)
          ...
          ${bankFilter.sql}
        ORDER BY at_pstdate DESC`,
      [grossPence, payoutDate, ...bankFilter.params],
    )) as unknown as OperaDupRow[];
    return Array.isArray(rows) && rows.length > 0 ? rows[0]! : null;
  } catch {
    return null;
...
function pickRawPayouts(payouts: Array<Record<string, unknown>>): RawPayout[] {
  return payouts.map((p) => ({
    ...
    arrival_date:
      typeof p.arrival_date === 'string' ? p.arrival_date : undefined,
  }));
}
...
export async function fetchGocardlessApiPayouts(
  ...
  environment: 'sandbox' | 'live',
  input: FetchApiPayoutsInput,
): Promise<FetchApiPayoutsResponse> {
  ...
  for (const payout of payoutsToFetch) {
    ...
    if (!fullPayout) continue;

    try {
      const isForeignCurrency =
        fullPayout.currency.toUpperCase() !== homeCurrency.toUpperCase();

      let possibleDuplicate = false;
      let isDefiniteDuplicate = false;
      let isValueMismatch = false;
      let bankTxWarning: string | null = null;
      ...
      try {
        if (isForeignCurrency) {
          ...
          if (row) {
            ...
            bankTxWarning = `Already posted - ref '${refSuffixValue}' found: £${(
              Number(row.ae_value) / 100
            ).toFixed(2)} on ${dateStr} (note: foreign currency, GBP equivalent)`;
          }
        } else {
          // GBP path
          const row = await findOperaByReference(operaDb, refSuffixValue, bankFilter);
          if (row) {
            ...
            if (Math.abs(operaValuePence - grossPence) <= 100) {
              isDefiniteDuplicate = true;
              possibleDuplicate = true;
              bankTxWarning = `Already posted - ref '${refSuffixValue}': £${(
                operaValuePence / 100
              ).toFixed(2)} on ${dateStr}`;
            } else {
              isValueMismatch = true;
              bankTxWarning = `Value mismatch - ref '${refSuffixValue}' in Opera: £${(
                operaValuePence / 100
              ).toFixed(2)} on ${dateStr}, GC gross: £${(grossPence / 100).toFixed(2)}`;
            }
          }

          if (!possibleDuplicate && grossPence > 0 && fullPayout.arrival_date) {
            const gross = await findOperaByGrossDate(
              operaDb,
              grossPence,
              fullPayout.arrival_date,
              bankFilter,
            );
            if (gross) {
              possibleDuplicate = true;
              ...
              const ref =
                (gross.ae_entref ?? '').toString().trim() || 'N/A';
              bankTxWarning = `Already posted - gross amount: £${(
                Number(gross.ae_value) / 100
              ).toFixed(2)} on ${dateStr} (ref: ${ref})`;
            }
          }
        }
      } catch {
      ...
      const allPayments: PaymentInput[] = fullPayout.payments.map((p) => ({
      ...
      let matchedPayments: Array<Record<string, unknown>> = allPayments.map(
      ...
      if (allPayments.length > 0) {
        ...
        if (matchResult.success) {
          matchedPayments = matchResult.payments.map(
            (p) => ({ ...p }) as Record<string, unknown>,
          );
        }
      }
