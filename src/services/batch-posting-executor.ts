      ...
      await executeWithDeadlockRetry(operaDb, async (trx) => {
        ...
        if (transferResult) {
          ...
          if (transferResult.ntranWritten) {
            const rows = (await trx.raw(
              `SELECT COUNT(*) AS cnt, SUM(nt_value) AS total
              ...
              | Array<{ cnt: number | null; total: number | null }>
              | { recordset?: Array<{ cnt: number | null; total: number | null }> };
            const row = Array.isArray(rows)
              ? rows[0]
              : (rows as { recordset?: Array<{ cnt: number | null; total: number | null }> }).recordset?.[0];
            const cnt = Number(row?.cnt ?? 0);
            const total = Number(row?.total ?? NaN);
            if (cnt !== 2) {
              throw new PostingVerificationError(
                `transfer ntran rows missing (expected 2 across pstids ` +
                  `${transferResult.ntranPstidSource}/${transferResult.ntranPstidDest}): got ${cnt}`,
                { batchRef: transferResult.sourceEntry, phase: 'in-trx' },
              );
            }
            if (!Number.isFinite(total) || Math.abs(total) > 0.005) {
              throw new PostingVerificationError(
                `transfer ntran does not balance across pstids ` +
                  `${transferResult.ntranPstidSource}/${transferResult.ntranPstidDest}: sum=${total}`,
                { batchRef: transferResult.sourceEntry, phase: 'in-trx' },
              );
            }
          }
