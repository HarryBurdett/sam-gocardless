...
export async function listMandates(appDb, opts = {}) {
    try {
        ...
        const filtered = rows.filter((r) => r.opera_account !== '__UNLINKED__' ||
            !linkedIds.has((r.mandate_id ?? '').trim()));
        // Sort by opera_name (case-insensitive)
        filtered.sort((a, b) => (a.opera_name ?? '').toLowerCase().localeCompare((b.opera_name ?? '').toLowerCase()));
...
async function detectRelink(appDb, mandateId, newOperaAccount) {
 ...
 *     supplied.
 */
export async function linkMandate(appDb, input) {
    ...
    if (oldAccount && !input.confirm) {
        return {
            ...
            error: `This mandate is currently linked to ${oldAccount}. ` +
    ...
    try {
        ...
        if (input.gocardlessCustomerId !== undefined &&
...
export function findOperaCustomerMatch(gcName, customers) {
    const normGc = normaliseCompanyName(gcName);
    if (!normGc)
        return null;
    for (const c of customers) {
        if (normaliseCompanyName(c.name) === normGc)
            return c;
    }
    for (const c of customers) {
 ...
 */
export async function syncMandatesFromGocardless(appDb, fetchPage, fetchCustomer, operaCustomers) {
    try {
        let synced = 0;
        let newCount = 0;
        let updated = 0;
        ...
        while (true) {
            ...
            if (!page.mandates || page.mandates.length === 0)
                break;
        ...
        const allRows = (await appDb('gocardless_mandates').select('id', 'mandate_id', 'opera_account'));
        ...
        if (autoLinked > 0)
            message += ` (${autoLinked} auto-linked to Opera)`;
        if (newCount > 0)
            message += `, ${newCount} new`;
        if (updated > 0)
...
export async function cancelMandate(appDb, mandateId, cancelRemote) {
    const id = (mandateId ?? '').trim();
    if (!id)
        return { success: false, error: 'mandate_id is required' };
...
export async function unlinkMandate(appDb, mandateId) {
    const id = (mandateId ?? '').trim();
    if (!id)
        return { success: false, error: 'mandate_id is required' };
    try {
...
export async function listUnlinkedMandates(appDb) {
    try {
        const rows = (await appDb('gocardless_mandates').where({
            opera_account: '__UNLINKED__',
        }));
        rows.sort((a, b) => (a.opera_name ?? '').toLowerCase().localeCompare((b.opera_name ?? '').toLowerCase()));
