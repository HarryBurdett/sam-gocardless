...
export async function checkDuplicateBatch(operaDb, input) {
    const result = {
    ...
    if (bankRef) {
        try {
            ...
            if (refRow) {
                ...
                result.ref_warning = `Already imported: ref '${bankRef}' on ${formatDateDmy(refRow.at_pstdate ?? null)}`;
            }
        }
        catch {
    ...
    if (!result.ref_warning) {
        try {
            let q = operaDb('atran')
                .join('aentry', function joinAentry() {
                this.on('ae_acnt', '=', 'at_acnt')
                    .andOn('ae_cntr', '=', 'at_cntr')
                    .andOn('ae_cbtype', '=', 'at_cbtype')
                    .andOn('ae_entry', '=', 'at_entry');
            ...
            if (dupRow) {
                ...
                result.duplicate_warning = `Cashbook entry found: £${value.toFixed(2)} on ${formatDateDmy(dupRow.at_pstdate ?? null)} (ref: ${ref})`;
            }
        }
        catch {
            // advisory
    ...
    if (input.paymentDate) {
        try {
            ...
            if (grossRow) {
                ...
                const existingRef = (grossRow.ae_entref ?? '').toString().trim() ||
                ...
                if (bankRef &&
                    ...
                    result.bank_tx_warning = `Already posted - gross amount: £${value.toFixed(2)} on ${dateStr} (ref: ${existingRef})`;
                    result.possible_duplicate = true;
                }
                else {
                    result.bank_tx_warning = `Similar amount found: £${value.toFixed(2)} on ${dateStr} (ref: ${existingRef}) - verify before importing`;
                }
            }
        }
        catch {
    ...
    if (!result.ref_warning &&
        ...
        try {
            const rows = (await operaDb('atran')
                ...
                .select('at_acnt', 'at_cntr', 'at_cbtype', 'at_entry', operaDb.raw('SUM(at_value) as entry_total'), operaDb.raw('MIN(at_pstdate) as entry_date'), operaDb.raw('COUNT(*) as line_count')));
            const sortedGcAmounts = input.payments
            ...
            for (const entry of rows) {
                const lines = (await operaDb('atran')
                    ...
                    .whereIn('at_type', [1, 4, 6])
                    .andWhere('at_value', '>', 0)
                    .select('at_value', 'at_name'));
                if (lines.length === 0)
                    continue;
                const entryAmounts = lines
                    .map((l) => Number(l.at_value ?? 0))
                    .sort((a, b) => a - b);
                if (entryAmounts.length !== sortedGcAmounts.length)
                    continue;
                const allMatch = entryAmounts.every((v, i) => Math.abs(v - (sortedGcAmounts[i] ?? 0)) <= 1);
                if (allMatch) {
                    const total = Number(entry.entry_total ?? 0) / 100;
                    const dateStr = formatDateDmy(entry.entry_date);
                    result.bank_tx_warning = `Already posted - batch: ${entryAmounts.length} payments totaling £${total.toFixed(2)} on ${dateStr}`;
                    result.possible_duplicate = true;
                    break;
                }
            }
        ...
        try {
            for (const payment of input.payments.slice(0, 5)) {
                ...
                const row = (await operaDb('atran')
                    ...
                    .andWhere(function gcRefFilter() {
                    this.where('at_refer', 'like', '%GC%').orWhere('at_refer', 'like', '%GoCardless%');
                })
                    .orderBy('at_pstdate', 'desc')
                ...
                if (row) {
                    ...
                    result.bank_tx_warning = `Already posted - payment: £${value.toFixed(2)} (${name}) on ${dateStr} with GC ref`;
                    result.possible_duplicate = true;
                    break;
                }
            }
        ...
        try {
            ...
            if (feesRow) {
                ...
                if (!result.bank_tx_warning) {
                    result.bank_tx_warning = `Already posted - fees: £${value.toFixed(2)} on ${dateStr} (ref: ${ref})`;
                }
                else {
                    result.bank_tx_warning += ` | Fees also posted: £${value.toFixed(2)} on ${dateStr}`;
                }
                result.possible_duplicate = true;
            }
        }
