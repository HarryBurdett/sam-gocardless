...
async function recordImportHistory(
  ...
  for (let attempt = 0; attempt < backoffsMs.length; attempt += 1) {
    ...
    try {
      ...
      if (attempt > 0) {
        ...
        console.warn(
          `[gocardless] recordImportHistory succeeded on attempt ${attempt + 1} for payout ${args.payoutId}`,
        );
      }
      return;
    } catch (err) {
      ...
      // eslint-disable-next-line no-console
      console.warn(
        `[gocardless] recordImportHistory attempt ${attempt + 1} failed for payout ${args.payoutId}: ${
  ...
  throw new Error(
    `Opera batch committed for payout ${args.payoutId} but audit row write to gocardless_imports failed after ${backoffsMs.length} attempts. Manual reconciliation REQUIRED — the next attempt to import this payout will silently double-post unless the audit row is added by hand. Last error: ${
  ...
  try {
    ...
    try {
      ...
      return {
        success: false,
        error:
          `Opera batch posted but SAM audit row failed. ${
